...
def _normalise_author(author: dict, expected_scholar_id: str) -> dict:
    ...
    author["scholar_id"] = expected_scholar_id
    author["updated"] = datetime.now(timezone.utc).isoformat()
    author["source"] = "SCHOLARLY_1_5_1"
    return author


def _load_previous_citation_count(expected_scholar_id: str) -> int | None:
    ...
    if previous_id and previous_id != expected_scholar_id:
        return None
    citedby = author.get("citedby")
    if isinstance(citedby, bool) or not isinstance(citedby, int):
        return None
    return citedby


def _validate_citation_change(scholar_id: str, citedby: int) -> None:
    previous_citedby = _load_previous_citation_count(scholar_id)
    if previous_citedby is None or citedby >= previous_citedby:
        return
...
def fetch_once(output_dir: Path) -> None:
    # Import in the worker process so a blocked request can be terminated cleanly.
    ...
    _validate_citation_change(scholar_id, author["citedby"])

...
1. 配置谷歌学术引用爬虫：
    ...
    1. 在 GitHub 本仓库的 `Actions` 页面中，点击 *"I understand my workflows, go ahead and enable them"* 启用工作流。该工作流会将 Google Scholar 引用数据 `gs_data.json` 写入 `google-scholar-stats` 分支，每天 08:17 UTC 自动运行，也可以在 Actions 页面手动触发。
